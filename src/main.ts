...
import { Character, CharacterIndicesByName } from "./interfaces";
...
async function getCharacters(
    dir: string = gameDir
): Promise<[Character[], CharacterIndicesByName]> {
    const characters: Character[] = [];
    const characterIndicesByName: CharacterIndicesByName = {};
    ...
    charactersTxt.forEach((character: string, index: number) => {
        if (fs.existsSync(path.join(dir, "data", "dats", character + ".dat"))) {
            ...
            characterIndicesByName[character] = index;
        }
    });
    ...
    lockedTxt.forEach((locked: string) => {
        characters[
            characterIndicesByName[locked]
        ].randomSelection = false;
    });
    return [characters, characterIndicesByName];
    // const [characters, characterIndicesByName] = await getCharacters();
}
...
async function writeCharacterRandom(
    ...
    lockedTxt.shift();
    if (randomSelection) {
        lockedTxt = lockedTxt.filter((locked) => locked != character);
    } else {
