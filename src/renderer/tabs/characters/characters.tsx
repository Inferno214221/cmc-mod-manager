...
const styles: typeof import("../../app/app.css") &
  typeof import("./characters.css") = Object.assign(
  {},
  appStyles,
  charactersStyles,
);
...
export function TabCharacters({
  ...
  const [altTarget, setAltTarget]: [
    Character | null,
    Dispatch<SetStateAction<Character | null>>,
  ] = useState(null);
  ...
  async function writeAltsAsCharacters(value: boolean | null): Promise<void> {
    ...
    setOperations((prev: Operation[]) => {
      ...
      operationId =
        newOperations.push({
          title: value
            ? message("operation.alt.include.started.title")
            : message("operation.alt.exclude.started.title"),
          body: value
            ? message("operation.alt.include.started.body")
          ...
          call: async () => {
            ...
            setOperations(
              finishOp(
                operationId,
                value
                  ? message("operation.alt.include.finished.body")
    ...
    setOperations((prev: Operation[]) => {
      ...
      operationId =
        newOperations.push({
          ...
          body: message(
            "operation.character.randomSelection.started.body",
            character.name,
          ),
          ...
          call: async () => {
            ...
            setOperations(
              finishOp(
                operationId,
                message(
                  "operation.character.randomSelection.finished.body",
                  character.name,
                ),
              ),
            );
    ...
    <tr className={styles.characterDisplayRow}>
                ...
                setOperations((prev: Operation[]) => {
                  ...
                  operationId =
                    newOperations.push({
                      ...
                      call: async () => {
                        const extractDir: string = await api.extractCharacter(
                          character.name,
                        );
                        setOperations(
                          finishOp(
      ...
      <td>
        <div className={styles.characterDisplayAlts}>
          {character.alts
            .filter((alt: Alt) => alt.base != alt.alt)
          ...
          onClick={async () => {
            ...
            setOperations((prev: Operation[]) => {
              ...
              operationId =
                newOperations.push({
                  title: message(
                    "operation.character.seriesDeletion.started.title",
                  ),
                  body: message(
            ...
            api.getGameDir().then((gameDir: string) => {
              api
                .pathJoin(gameDir, "gfx", "seriesicon", series + ".png")
                .then((path: string) => {
                  setOperations((prev: Operation[]) => {
                    const newOperations: Operation[] = [...prev];
